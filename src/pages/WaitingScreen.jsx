...
export default function WaitingScreen({ message = "Please wait..." }) {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(containerRef.current, { opacity: 0, scale: 0.97 }, { opacity: 1, scale: 1, duration: 0.8, ease: "power3.out" });
    dotsRef.current.forEach((dot, i) => {
      gsap.to(dot, { opacity: 0.3, yoyo: true, repeat: -1, delay: i * 0.2, duration: 0.6, ease: "sine.inOut" });
    });
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 justify-center mt-2">
        {[0, 1, 2].map((i) => (
          <div key={i} ref={(el) => (dotsRef.current[i] = el)} className="w-3 h-3 rounded-full bg-pink-400"></div>
        ))}
      </div>
    </div>
  );
